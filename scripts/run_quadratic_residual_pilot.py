#!/usr/bin/env python3
"""Run the controlled symbolwise quadratic-residual WSPR-2 pilot."""
...
from validate_quadratic_residual_pilot import main as validate_trajectory
...
A_RES_GRID_HZ = (0.0, 0.125, 0.25, 0.375, 0.5, 0.75, 1.0)
N_PER_CONDITION = 100
EXPECTED_CONDITIONS = 28
EXPECTED_TOTAL_TRIALS = 2800
BASE_SEED = 2026092700
...
OUTPUT_DIR = ROOT / "results" / "csv" / "quadratic_residual_pilot"
TRIAL_CSV = OUTPUT_DIR / "quadratic_residual_pilot_trials.csv"
SUMMARY_CSV = OUTPUT_DIR / "quadratic_residual_pilot_summary.csv"
MATRIX_CSV = OUTPUT_DIR / "quadratic_residual_pilot_matrix.csv"

TRIAL_FIELDS = (
    "snr_db", "A_res_hz", "trial_index", "seed", "decoded_success",
    "decoded_message", "reported_snr", "reported_drift", "reported_frequency",
    "runtime_seconds", "decoder_exit_status", "wsprd_version",
    "decoder_stdout", "decoder_stderr",
)
SUMMARY_FIELDS = (
    "snr_db", "A_res_hz", "n", "successes", "p_decode",
...
def impaired_waveform(amplitude_hz: float):
    ...
    active = wspr_symbol_quadratic_residual_trajectory(162, 256, amplitude_hz)
...
def run_trial(task: tuple[float, float, int, int]) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="wspr-quadratic-pilot-") as directory:
        work = Path(directory)
...
def validate_specification() -> None:
    ...
    assert len(A_RES_GRID_HZ) == 7
    assert N_PER_CONDITION == 100
    assert len(SNR_GRID) * len(A_RES_GRID_HZ) * N_PER_CONDITION == EXPECTED_TOTAL_TRIALS == 2800
...
def validate_trials(rows: list[dict[str, str]]) -> None:
    ...
    for snr_db in SNR_GRID:
        seed_sets = [
            {int(row["seed"]) for row in grouped[(snr_db, amplitude_hz)]}
            for amplitude_hz in A_RES_GRID_HZ
        ]
...
def main() -> int:
    ...
    print("NOMINAL_LINEAR_DRIFT_HZ=0")
    print("TRAJECTORY=wspr_symbol_quadratic_residual_trajectory(162, 256, A_res_hz)")
    print("TOTAL_TRIALS")
...
if __name__ == "__main__":
    try:
        ...
        print(f"quadratic residual pilot failed: {error}", file=sys.stderr)
        raise SystemExit(1)
